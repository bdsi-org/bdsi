...
const Book: React.FC<BookProps> = ({ book }) => {
  ...
  const handleDownload = async () => {
    try {
      ...
      const response = await fetch(`https://api.buddhadhammaschool.org/books/${book.id}/count`);
      const data = await response.json();
      ...
      if (response.ok) {
        ...
        const updateResponse = await fetch(`https://api.buddhadhammaschool.org/books/${book.id}/count`, {
          method: 'PUT',
          headers: {
